...
public class Matrix implements ActionListener
{
     ...
     *    for taking matrix's dimensions
     ...
     *
     * 4- private void ChooseOperation () for choosing an operation to be applied
     *    to the matrix
     *
     ...
     *    Do an addition operation of matrix with other matrix
     *
     ...
     *    Do a multiplication operation between matrix and a number
     *    by using multliplyByScalar method and show that for the user
     *
     * 13 - private static double [][] multliplyByScalar (double [][] matrix , double x)
     *    but only inner the program
     *
     * 14 - private static void divideByScalar ()
     *      Do a dividing operation of matrix and other matrix
     *
     * 15 - private static void guiGetValue ()
     *      getting determinent values by using getValue method
     ...
     *      for swapping two rows in order using to to get determination's value
     *
    ...
    private void ChooseOperation ()
    {
        ...
        nDivisionB = new JButton ("Dividing by scalar");
        ...
        if(col == row )
        {
            getValueB = new JButton ("DET Value");
            ...
            LUFactorization = new JButton("LU Factorization");
            LUFactorization.setPreferredSize(new Dimension(175,35));
            LUFactorization.addActionListener(this);
            choosePanel[5].add(LUFactorization);
    ...
    public  void actionPerformed(ActionEvent e)
    {
        ...
        else   if(e.getSource() == LUFactorization)
        {
            LU();
        }
    ...
    private static void matrixPlusMatrix ()
    {
        ...
        else
        {
            ...
            if(setElements(m2, "Fill Aditional Matrix"))
            {
                ...
                showMatrix(sum, "Summa tion Result");
            }
        }//end else checking
    }//end plus matrix
    ...
    private static void LU () {
        double[][] LMatrix = new double[myMatrix.length][myMatrix.length];
        float[][] UMatrix = new float[myMatrix.length][myMatrix.length];
        float[][] MMatrix = new float[myMatrix.length][myMatrix.length];
        float[][] CMatrix = new float[myMatrix.length][myMatrix.length];
        ...
        for (int i = 0; i <= (myMatrix.length - 1); i++) {
            for (int j = 0; j <= (myMatrix.length - 1); j++) {
                //i and j initial value zero or 1
                if (i == j) {
                    LMatrix[i][j] = 1;
                } else LMatrix[i][j] = 0;
            }
        ...
        for (int i = 0; i <= (myMatrix.length - 2); i++)
        {
            ...
            if (myMatrix[i][i] == 0) JOptionPane.showMessageDialog(null, "Zero element on diagonal!");



            // initialization of M with identity matrix
            for (int i2 = 0; i2 <= (myMatrix.length - 1); i2++) {
                for (int j2 = 0; j2 <= (myMatrix.length - 1); j2++) {
                    if (i2 == j2) MMatrix[i2][j2] = 1;
                    else MMatrix[i2][j2] = 0;
                }
            }


            for (int j = i + 1; j <= (myMatrix.length - 1); j++) {
                MMatrix[j][i] = (float) (-1 * myMatrix[j][i] / myMatrix[i][i]);
                LMatrix[j][i] = (float) (myMatrix[j][i] / myMatrix[i][i]);

            }


            for(int ic = 0;ic<=(myMatrix.length-1);ic++)
            {
                for (int jc=0; jc<=(myMatrix.length-1); jc++) {
                    CMatrix[ic][jc] = 0;
                }
            }

            //Multiplication of Matrixes M and A
            for(int ii=0;ii<=(myMatrix.length-1);ii++)
            {
                for(int jj=0;jj<=(myMatrix.length-1);jj++)
                {
                    for (int kk=0; kk<=(myMatrix.length-1); kk++) {
                        CMatrix[ii][jj] = (float)(CMatrix[ii][jj] + (MMatrix[ii][kk] * myMatrix[kk][jj])) ;
                    }
                }
            }

            //assigning C to A
            for(int id = 0;id<=(myMatrix.length-1);id++)
            {
                for (int jd=0; jd<=(myMatrix.length-1); jd++) {
                    myMatrix[id][jd] = CMatrix[id][jd];
                }
            }

        }

        showMatrix(myMatrix,"U factor");
        showMatrix(LMatrix,"L Factor");
    ...
    for(j=1; j<=myMatrix.length; j++) /* loop for the generation of upper triangular matrix*/
    {
    ...
    x[myMatrix.length]=A[myMatrix.length][myMatrix.length+1]/A[myMatrix.length][myMatrix.length];
    /* this loop is for backward substitution*/
    for(i=myMatrix.length-1; i>=1; i--)
    {
        sum=0;
        for(j=i+1; j<=myMatrix.length; j++)
        {
            sum=sum+A[i][j]*x[j];
        }
        x[i]=(A[i][myMatrix.length+1]-sum)/A[i][i];
    }
    cout<<"\nThe solution is: \n";
    for(i=1; i<=myMatrix.length; i++)
    {
